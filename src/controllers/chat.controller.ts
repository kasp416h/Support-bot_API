...
async function getChatGptResponse(req: Request, res: Response): Promise<void> {
  ...
  if (!businessInfo?.contactInfo?.phone || !businessInfo?.contactInfo?.email) {
    res.json({
      response:
        "The chatbot is currently unavailable. Please ensure both phone and email are set for the business.",
    });
    return;
  }
